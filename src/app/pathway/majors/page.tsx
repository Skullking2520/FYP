...
import { getJobMajors } from "@/lib/backend-api";
import { clearSelectedMajor, loadSelectedJob, saveSelectedMajor } from "@/lib/pathway-storage";
import type { BackendMajorRecommendation } from "@/types/api";
      ...
      <header className="rounded-2xl border bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-sm uppercase tracking-wide text-slate-500">Step 2</p>
            <h1 className="text-2xl font-semibold text-slate-900">Select a major</h1>
            <p className="mt-2 text-sm text-slate-600">Recommended majors for your selected job.</p>
              ...
              <li key={String(major.major_id)} className="rounded-xl border p-4">
                <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                  ...
                  <button
                    ...
                    onClick={() => {
                      saveSelectedMajor({ major_id: String(major.major_id), major_name: major.major_name });
