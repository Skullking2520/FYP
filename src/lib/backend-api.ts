...
export async function getMajorSkills(major_id: string): Promise<BackendMajorSkill[]> {
  return backendFetch<BackendMajorSkill[]>(`/api/majors/${encodeURIComponent(major_id)}/skills`, { method: "GET" });
}

export async function getMajorGaps(major_id: string, skill_keys: string[]): Promise<BackendMajorSkill[]> {
  const payload: MajorGapsRequest = { skill_keys };
