...
def get_major_skills(major_id: int) -> list[MajorSkillItem]:
    ...
    try:
        ...
        # If the normalized major_skill table is empty (common in read-only DB
        # setups), derive skills from ESCO occupation-skill links instead.
        if not rows:
            derived_sql = """
            SELECT
              s.id AS skill_id,
...
def get_major_programs(
    ...
    Ranking combines:
    - skill overlap between major_skill and program_skill (score)
